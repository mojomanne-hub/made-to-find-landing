...
function Button({ children, secondary = false }: { children: React.ReactNode; secondary?: boolean }) {
  return <a href="#start" className={secondary ? "button secondary" : "button"}>{children}{!secondary && <ArrowRight size={17} />}</a>;
}
...
export default function Home() {
  return <main id="top">
    <nav className="nav"><Logo /><div className="nav-links"><a href="#funktion">So funktioniert&apos;s</a><a href="#features">Features</a><a href="#einsatz">Einsatzbereiche</a></div><div className="nav-actions"><a className="login" href="/login">Einloggen</a><Button>Kostenlos starten</Button></div></nav>
    ...
    <section className="hero"><div className="hero-glow" /><div className="hero-grid">
      <motion.div className="hero-copy" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7 }}>
        ...
        <div className="hero-buttons"><Button>Kostenlos starten</Button><Button secondary>So funktioniert&apos;s <ChevronRight size={17} /></Button></div>
    ...
    <section id="start" className="final-cta"><div className="cta-glow" /><p className="eyebrow">Alles bereit?</p><h2>Finde alles.<br /><em>Jederzeit.</em></h2><p>Bring Ordnung in dein Zuhause, deine Werkstatt oder dein Lager.</p><Button>Kostenlos starten</Button><small>Einfach starten. Jederzeit den Überblick behalten.</small></section>
    <footer><Logo /><span>© {new Date().getFullYear()} Made to Find</span><div><a href="#">Datenschutz</a><a href="#">Impressum</a><a href="#">Kontakt</a></div></footer>
  </main>;
}
